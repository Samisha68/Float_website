...
let keys;
const jwks = () => (keys ??= createRemoteJWKSet(new URL(`https://auth.privy.io/api/v1/apps/${encodeURIComponent(appId())}/jwks.json`), { timeoutDuration: 5000 }));
let client;
...
export default createWaitlistHandler({
  ...
  verify: async token => (await jwtVerify(token, jwks(), { issuer: "privy.io", audience: appId(), algorithms: ["ES256"], requiredClaims: ["exp", "iat", "sub"] })).payload,
  getEmail: async id => {
